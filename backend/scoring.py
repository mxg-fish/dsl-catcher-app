...
def calc_week_scores(week_id: int) -> list[dict]:
    with get_conn() as conn:
        ...
        for p in players:
            ...
            # Recibir (SL+)
            sl = conn.execute(
                "SELECT sl_plus, shadow_strike_pct FROM weekly_sl WHERE week_id=? AND player_id=?",
                (week_id, pid)
            ).fetchone()
            if sl and sl["sl_plus"]:
                s["sl_plus"] = sl["sl_plus"]
                s["recibir"] = round((sl["sl_plus"] / league_avg) * MAX_RECIBIR, 2)
            ...
            if game_ids:
                ...
                # bp_outs and CS are flat bonus points regardless of throw volume
            ...
            s["total"] = round(s["liderazgo"] + s["recibir"] + s["practica"] + s["blockear"] + s["tirar"], 2)
            results.append(s)
